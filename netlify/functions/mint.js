...
// Fungsi ini sekarang menangani 402 CHALLENGE DAN 200 SUCCESS (MINT)
...
// ABI MINIMAL untuk fungsi mint(address, string) - SESUAIKAN DENGAN KONTRAK ANDA!
const NFT_ABI = [
    {
        "inputs": [
            {
...
// Kunci Privat Minter harus diatur di Environment Variables Netlify
...
exports.handler = async (event, context) => {
  ...
  // --- LOGIKA UTAMA: Cek apakah Klien sudah membayar (ada header X-PAYMENT) ---
  if (xPaymentHeader) {
      console.log("Found X-PAYMENT header. Attempting verification and mint...");

      // 1. VERIFIKASI PEMBAYARAN X402
      let payerAddress;
      try {
          // DEKODING PAYLOAD X402
          const decodedPayload = JSON.parse(Buffer.from(xPaymentHeader, 'base64').toString());
          
          // !!! PENTING: LOGIKA VERIFIKASI ON-CHAIN SEHARUSNYA DI SINI !!!
          // Anda harus menggunakan 'x402 facilitator' atau kode Anda sendiri untuk:
          // a) Memverifikasi tanda tangan/keabsahan payload
          // b) Memastikan transaksi on-chain yang diacu (proof) benar-benar sukses (settled)
          
          // ASUMSI SIMPLIFIKASI (TIDAK AMAN UNTUK PRODUKSI)
          // Kami mengasumsikan payerAddress ada dalam payload dan sah.
          payerAddress = decodedPayload.payerAddress; 
          if (!payerAddress) throw new Error("Payer address not found in payload.");

          console.log(`✅ Payment proof accepted from: ${payerAddress}`);
      ...
      // 2. PICU MINT NFT (ON-CHAIN ACTION)
      try {
          const nftContract = new ethers.Contract(NFT_CONTRACT_ADDRESS, NFT_ABI, wallet);
          
          // Gunakan hash unik atau ID token untuk URI
          const tokenURI = `${BASE_TOKEN_URI}${Date.now()}`; 

          console.log(`✨ Minting NFT to ${payerAddress}...`);

          const tx = await nftContract.mint(payerAddress, tokenURI);
  ...
  // --- LOGIKA DEFAULT: Kirim Permintaan Pembayaran 402 ---
  else {
      console.log("🚀 No X-PAYMENT header found → returning 402 Payment Required...");

      // URL publik resource (harus menunjuk ke fungsi ini sendiri: /mint)
      const resourceUrl = `https://${event.headers.host}${event.path}`; // Gunakan event.path agar menunjuk ke /mint.js
      ...
      const paymentMethod = {
        ...
        resource: resourceUrl, // resource target → MINT.JS ITU SENDIRI
        ...
        payTo: X402_RECIPIENT_ADDRESS, // alamat penerima
      ...
      return {
        ...
        headers: {
          'Content-Type': 'application/json',
          'Access-Control-Allow-Origin': '*',
          'Access-Control-Allow-Headers': 'Content-Type, X-PAYMENT', // Tambahkan X-PAYMENT
          'Access-Control-Allow-Methods': 'GET, OPTIONS, POST' // Tambahkan POST/GET sesuai kebutuhan
        }
      };
  }
};
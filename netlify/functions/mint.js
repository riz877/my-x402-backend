...
// Base Block Explorer URL for transaction links (since asset is on Base)
const BASE_EXPLORER_URL = 'https://basescan.org/tx/';

// Setup provider and the relayer wallet (which pays gas)
const provider = new JsonRpcProvider(PROVIDER_URL);
const relayerWallet = new Wallet(RELAYER_PRIVATE_KEY, provider);

// Required ABIs
// USDC ABI: Only contains the 'transferWithAuthorization' function for gasless payment
const usdcAbi = [
  {
    "constant": true,
    "inputs": [],
...
// NFT ABI: Only contains the 'mint' function
const nftAbi = [
    {
        "inputs": [
            {
...
module.exports.handler = async (event, context) => {
  ...
  // --- GET Logic: Return 402 Payment Required (Pricing) ---
  if (event.httpMethod === 'GET') {
    console.log("🚀 Function 'mint' triggered (GET) → returning 402 Payment Required...");

    // Construct the resource URL dynamically to point back to this function
    const resourceUrl = `https://${event.headers.host}${event.path}`;

    const paymentMethod = {
      scheme: "exact",
      network: "base",
      maxAmountRequired: "2000000", // 2 USDC (2 * 10^6)
      resource: resourceUrl, // This is the POST endpoint for payment
      ...
      payTo: "0xD95A8764AA0dD4018971DE4Bc2adC09193b8A3c2", // Recipient wallet address
      maxTimeoutSeconds: 600,
      asset: "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", // USDC (Base Network)
      outputSchema: {
  ...
  // --- POST Logic: Process Payment & Minting ---
  if (event.httpMethod === 'POST') {
    ...
    // Input validation (from x402 scan payload)
    ...
    try {
      ...
      // 1. Transfer USDC using the user's signature (Gasless payment)
      ...
      // 2. Mint NFT after successful payment
      ...
      // Generate transaction links for the client
      ...
      return {
        ...
        headers,
        body: JSON.stringify({
          message: 'Claim successful! Payment processed and NFT minted.',
